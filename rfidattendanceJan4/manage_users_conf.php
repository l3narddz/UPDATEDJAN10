      ...
      exit();
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if ($row['add_card'] == 0) {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email) &&!empty($Section)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            $sql="UPDATE users SET username=?, serialnumber=?, sectionName=?, gender=?, email=?, user_date=CURDATE(), device_uid=?, device_dep=?, add_card=1 WHERE id=?";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                ...
                                mysqli_stmt_bind_param($result, "sdssssi", $Uname, $Number, $Section, $Gender, $Email, $dev_uid, $dev_name, $user_id );
                                mysqli_stmt_execute($result);
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email)) {
                                ...
                                $sql="UPDATE users SET username=?, serialnumber=?, sectionName=?,gender=?, email=?, device_uid=?, device_dep=? WHERE id=?";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
